...
*              FIPS 204 §6.1 ML-DSA.KeyGen — deterministic.
*              Exposed as pqcrystals_dilithium5_ref_keypair_internal via
*              the DILITHIUM_NAMESPACE macro defined in sign.h.
*
* Arguments:   - uint8_t *pk:          pointer to output public key
*              - uint8_t *sk:          pointer to output secret key
*              - const uint8_t *seed:  pointer to 32-byte seed ξ
*
* Returns 0 (success)
*
* MXD patch: this function did not exist in the upstream snapshot.
* Added to expose a seeded (deterministic) keypair path so that
* mxd_pq01_keygen_at_leaf can derive ML-DSA-87 keys from a BIP-39/
* MXD-PQ-01 leaf ξ without touching randombytes().
**************************************************/
int crypto_sign_keypair_internal(uint8_t *pk, uint8_t *sk, const uint8_t seed[SEEDBYTES]) {
  ...
  /* Expand seed ξ into (rho, rhoprime, key) — deterministic, no randombytes */
  seedbuf[0] = seed[0]; /* copy seed into start of buf then expand in place */
  for(int i = 1; i < SEEDBYTES; i++) seedbuf[i] = seed[i];
