...
# Copy source code
COPY --chown=builder:builder . /home/builder/mxd/

# Build the library (no dependency installation needed)
WORKDIR /home/builder/mxd
RUN mkdir -p build && \
    cd build && \
...
COPY --from=builder --chown=mxdnode:mxdnode /home/builder/mxd/build/lib/libmxd.so /opt/mxd/lib/
COPY --from=builder --chown=mxdnode:mxdnode /home/builder/mxd/build/lib/mxd_node /opt/mxd/bin/
COPY --from=builder --chown=mxdnode:mxdnode /home/builder/mxd/src/node/default_config.json /opt/mxd/config/
