...
static void configure_tls_security(CURL* curl) {
    ...
    if (config && !config->bootstrap.verify_tls) {
        if (config->network_type && strcmp(config->network_type, "mainnet") == 0) {
            MXD_LOG_ERROR("http", "TLS verification cannot be disabled on mainnet - ignoring");
        } else {
            MXD_LOG_WARN("http", "TLS verification disabled by configuration - INSECURE (non-mainnet only)");
