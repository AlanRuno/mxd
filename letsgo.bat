...
if "%~1"=="" (
    echo Usage: %~nx0 ^<testnet^|mainnet^> [options]
    echo.
    echo Options:
    echo   --docker         Run via Docker image from ghcr.io/AlanRuno/mxd
    echo   --from-source    Compile everything from source (~10-20 min)
    echo   --reset          Wipe local node data before start
    echo   (none)           Default: download prebuilt release tarball + run
    echo.
    echo Examples:
    echo   %~nx0 testnet                 Download prebuilt and run (fastest)
    echo   %~nx0 testnet --docker        Pull + run via Docker
    echo   %~nx0 mainnet --from-source   Compile + run mainnet
    exit /b 1
)

set NETWORK_TYPE=%~1
REM Forward all remaining arguments (--docker / --from-source / --reset / etc.)
set EXTRA_ARGS=
shift
:collect_args
if "%~1"=="" goto args_done
...
echo.
"%BASH_EXE%" --login -c "cd '%SCRIPT_DIR%' && ./letsgo %NETWORK_TYPE%%EXTRA_ARGS%"
